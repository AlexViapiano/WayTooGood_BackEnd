...
// CUSTOMER: Imported for brands dropdown
  ...
  const getData = useCallback(
    async (signal) => {
      ...
      try {
        ...
        const adminRoles = roles.find(
          ({ name }) => name === "Super Admin" || name === "WTG"
        );

        if (
          queryInfos.endPoint ==
            "/content-manager/relations/application::product.product/brand" &&
          adminRoles == null
        ) {
          const id = get(auth.getUserInfo(), "id", "");
          var fullData = await request(
            strapi.backendURL + `/brands?created_by=` + id,
            {
              method: "GET",
            }
          );
