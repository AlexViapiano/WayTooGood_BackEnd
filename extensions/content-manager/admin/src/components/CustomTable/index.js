...
const CustomTable = ({
  ...
  var filteredHeaders = headers.filter((header) => {
    return header.name != "locales";
  });

  const colSpanLength =
    isBulkable && canDelete
  ...
  return (
    <Table className="table">
      <Headers headers={filteredHeaders} isBulkable={isBulkable && canDelete} />
      <tbody>
